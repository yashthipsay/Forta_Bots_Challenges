...
describe("Helper class test suite", () => {
    ...
    it("should return correct configuration data", async() => {
        ...
        mockProvider.addCallTo(mockConfigProxy, 0, Iface, "getConfiguration", {
          ...
          const getConfig = await helper.gettConfiguration(mockConfigProxy, mockTokenAddress, 0);
          ...
          const getConfigStringified = getConfig.map((item: any) => {
            if (Array.isArray(item)) {
              return item.map((subItem) => (ethers.BigNumber.isBigNumber(subItem) ? subItem.toString() : subItem));
            }
            return ethers.BigNumber.isBigNumber(item) ? item.toString() : item;
          });
          
          expect(JSON.stringify(getConfigStringified)).toBe(JSON.stringify(expectedConfig));
    })
    ...
    it("should return correct utilization data", async() => {
        ...
        const getUtilization = await helper.getUtilization(mockTokenAddress, "function getUtilization() public view returns (uint)", 0);
        expect(getUtilization.toString()).toBe(ethers.BigNumber.from(5000).toString());
    })
    ...
    it("should return correct supply APR", async() => {
        ...
        const getSupplyAPR = await helper.getSupplyAPR(mockTokenAddress, "function getSupplyRate(uint utilization) public view returns (uint64)", ethers.BigNumber.from(5000), 0);
    ...
    it("should return correct borrow APR", async() => {
        ...
        const getBorrowAPR = await helper.getBorrowAPR(mockTokenAddress, "function getBorrowRate(uint utilization) public view returns (uint64)", ethers.BigNumber.from(5000), 0);
